...
// Sell executes a sell operation
func Sell(cfg *config.Config, tokenMintAddress string, amount, slippage float64) error {
	...
	err = raydium.AddSwapInstruction(tx, pool, privateKey.PublicKey(), swapAmount, minOutputAmount, false)
	if err != nil {
	...
	log.Printf("Sell transaction sent: %s", txID)
	return nil
}
